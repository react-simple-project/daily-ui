          ...
          <div key={project.title}>
            <div className={styles.accordion}>
              <div className={styles['accordion__item']}>
                <div onClick={event => setValue(event.target.title)} title={+project.title.slice(-3)} className={styles['accordion__title']}>
                ...
                {+currValue === +project.title.slice(-3) && <div className={styles['accordion__content']}><a onClick={
                  event => onSelectProject(+event.target.alt.split(' ')[1])
                } href='#project'>{project.content}</a></div>}
              </div>
            </div>
          </div>)
        )
